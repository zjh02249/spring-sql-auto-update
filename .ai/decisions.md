...
## ADR-018: v1.3.6.1 SqlExecutor 命名与执行安全增强
...
为修复“执行后未恢复默认库/schema”问题，并提升跨 namespace 执行安全性，需要升级 `SqlExecutor` 实现。

### 决策

1. 将旧语义命名 `extractDatabaseName` 统一为 `extractNamespace`  
2. 扩展 namespace 提取语句覆盖：UPDATE/DELETE/INSERT/REPLACE/CREATE/ALTER/DROP/TRUNCATE  
3. 使用 JDBC `Savepoint` API 实现切换失败回滚  
4. 在语句执行 finally 路径中恢复默认 namespace，降低上下文污染风险

### 影响

- ✅ 正向：默认库/schema 恢复更可靠，事务安全性更好  
- ⚠️ 注意：namespace 标识符校验更严格，非法命名会快速失败（已通过测试验证）
...
完成上一个版本（1.3.5）的发布后，需要进行下一个版本（1.3.6）的发布。本次发布主要是版本号递增和常规维护。

### 决策

发布版本 1.3.6，主要包含：
1. **版本号更新**: 从 1.3.5 升级到 1.3.6
2. **文档同步**: 更新所有相关文档中的版本号
3. **模块发布**: 发布核心模块到 Maven 仓库
